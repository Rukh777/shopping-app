...
def orders(request):
    ...
    return render(request, 'app/orders.html',{'ordered_place':op})

def mobile(request, data=None):
    if data == None:
        mobiles = Product.objects.filter(category='M')
    elif data == 'OnePlus' or data == 'Apple':
...
def topwear(request, data=None):
    ...
    elif data == 'SASSAFRAS' or data == 'plusS':
        topwears = Product.objects.filter(category='TW').filter(brand=data)
    elif data == 'Roadster' or data == 'VeroModa':
        topwears = Product.objects.filter(category='TW').filter(brand=data)
    elif data == 'LaZoire': 
        topwears = Product.objects.filter(category='TW').filter(brand=data)
...
def bottomwear(request, data=None):
    ...
    elif data == 'PANIT' or data == 'Roadster':
        bottomwears = Product.objects.filter(category='BW').filter(brand=data)
    elif data == 'TokyoTalkies' or data == 'People':
        bottomwears = Product.objects.filter(category='BW').filter(brand=data)
...
class CustomerRegistration(View):
    ...
    def post(self, request):
        ...
        if form.is_valid():
            messages.success(request, 'Congartulations!! Registered Successfully ')
            form.save()
        return render(request, 'app/customerregistration.html',{'form':form})
